...
syscall_handler (struct intr_frame *f)
{
	...
	switch (syscall_no) {
		...
		case SYS_GET_SWAP_SIZE:
			...
			f->eax = block_size(swap_block) * BLOCK_SECTOR_SIZE;
			return;
		...
		case SYS_GET_SWAP_NAME:
			...
			strlcpy(name,block_name(swap_block),16);
		...
		case SYS_GET_SWAP_WRITE_CNT:
			...
			f->eax = get_write_cnt(swap_block);
			return;

		case SYS_SWAP_OUT:
			// TO DO

			frame_addr = (void*) ((int*)f->esp)[1];
			swap_out(frame_addr);
			f->eax = 0;
			return;
#endif
