  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      spte->virt_page_no =(uint32_t) ((uint32_t)upage / PGSIZE);
      spte->ofs = crt_ofs ;
      ...
      // Changes by Adrian Colesa - VM

      // The initial code that loaded the page from file was commented to delay the load until the page is really required
...
/* Get a page of memory. */
   uint8_t *kpage = palloc_get_page (PAL_USER);
      ...
      // Added by Adrian Colesa - VM
      printf("\n[lazy_loadingt] Virtual page %d: from offset %d in the executable file read %d bytes \ and zero the rest %d bytes\n",pg_no, crt->exec_file, spte->page_read_bytes, spte->page_zero_bytes);
     ...
     if (file_read (crt->exec_file, kpage, spte->page_read_bytes) != (int) spte->page_read_bytes)
      ...
      // Added by Adrian Colesa - Userprog + VM
      //printf("[load_segment] The process virtual page %d starting at virtual address 0x%x will be mapped onto the kernel virtual page %d (physical frame %d) starting at kernel virtual address 0x%x (physical address 0x%x)\n", ((unsigned int) upage)/PGSIZE, upage, (unsigned int)kpage/PGSIZE, ((unsigned int)vtop(kpage))/PGSIZE, kpage, vtop(kpage));
      printf("[lazy_loading] Virtual page %d (vaddr=0x%x): mapped onto the kernel virtual page %d (physical frame %d)\n", pg_no, upage, (unsigned int)kpage/PGSIZE, ((unsigned int)vtop(kpage))/PGSIZE);

	uint32_t my_upage=(uint32_t)upage-((uint32_t)upage%PGSIZE);
      /* Add the page to the process's address space. */
    if (!install_page ((void*)my_upage, kpage, spte->writable))
        {
          palloc_free_page (kpage);
          return false; 
        }

	printf("SUCCESFULLY INSTALLED %d", my_upage);
	return true;
